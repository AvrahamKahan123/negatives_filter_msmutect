...
@dataclass
class IndividualFileResult:
    total_mut_count: pd.DataFrame
    mut_count_per_motif: pd.DataFrame

def process_individual_file(fp, case_name) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
def process_and_save_individual_file(fp: str, case_name: str, filtered_save_path: str) -> IndividualFileResult:
    st = time.time()
    total_mut_counts, per_motif_mut_counts, filtered_muts_df = process_individual_file(fp, case_name)
    filtered_muts_df.to_csv(filtered_save_path, sep="\t", index=False)
    e = time.time()
    print(f"Processing time: {e - st}")
    return IndividualFileResult(total_mut_counts, per_motif_mut_counts)
...
def create_args_tuples(files: List[str], case_names: List[str], output_files_dir: str) -> List[Tuple[str, str, str]]:
    args_tuples = []
    for file, case in zip(files, case_names):
        args_tuples.append((file, case, os.path.join(output_files_dir, f"{case}.filtered.full.mut.tsv")))
    return args_tuples
